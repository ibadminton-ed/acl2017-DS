...
classification~\cite{go2009twitter}, tweet named entity classifying~\cite{ritter2011named}.
...
as a multi-instance classification problem under the \emph{at-least-one assumption}. 
...
Instead of characterizing the noise, these approaches only aim to reduce the bag influence of noise.
% In contrast to our work in noise modeling, these approaches all aim to identify either reliable or unreliable instances to reduce the bad influence of noise.
...
\paragraph{Neural Network Based Noise Modeling}
Recently, various approaches have been proposed in the computer vision domain for modeling the data noise using neural networks ~\cite{sukhbaatar2014training,reed2014training,chen2015webly,xiao2015learning,misra2016seeing}.
...
training instance.
However, instead of using Expectation-Maximization \red{ZW: What's the difference of EM and our CL?}, we train our model with a curriculum learning framework and trace regularization.
In the field of NLP, there is currently little work in neural-network-based noise modeling. \cite{fang2016learning} is the closest work, which uses a global transition matrix to model the noise introduced by
cross-lingual projection of training data. Our work advances \cite{fang2016learning} through dynamically generating a transition matrix for each instance,
providing a finer-grained, more effective way for modeling the noise. 

%which avoids the drawback of global transition matrix that it use a single transition matrix for both reliable and unreliable data.
