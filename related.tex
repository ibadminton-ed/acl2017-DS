...
The at-least-one assumption mentioned above is often a rather strong assumption in practice, and there are still chances that the
...
training data item. However, instead of using EM \todo{what's EM?}, we train our model with curriculum learning and trace regularization.
...
to dynamically adapt the transition matrix. This leads to significant better performance (see Section~\ref{evaluation}).
