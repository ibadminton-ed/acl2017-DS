...
A straightforward solution is to directly align the observed distribution, $\mathbf{o}$, with respect to the noisy annotations by minimizing \red{ZW: minimzing the product of corss entropy and regularization, i.e.?}:
...
The problem is at the beginning of the training, we have no prior about the noise pattern; thus, both  $\mathbf{T}$ and $\mathbf{p}$ are less reliable, which will make the training procedure is likely to fall into the local optimum.
...
Before proceeding to training details, we first discuss how we characterize \red{ZW: What??}
...
Initially, we set $\alpha=1$, and train our model  completely by minimizing the loss from the prediction distribution $\mathbf{p}$. That is, we do not expect to model the noise,  but focusing  on the prediction branch at this time. As the training progresses, the prediction branch gradually learns the basic prediction ability. We then increase the level by decreasing $\alpha$ and  $\beta$ by $0<\rho<1$ \todo{(need a formular? $\alpha=\alpha*(1-\rho)$??)} every $\tau$ epochs, i.e., learning more about the noise from the observed distribution $\mathbf{o}$ and allowing a relatively smaller $trace(\mathbf{T})$, to gradually shift part of our focus to \red{learning with noise}. This gradually learning
paradigm is a significant departure from prior work on noise modeling for \DS seen to date. 
