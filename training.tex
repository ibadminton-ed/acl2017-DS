...
\orange{this part is re-wrote.} The difficulty of training the transition matrix lies in that there is no direct guidance over the noise pattern. The only supervision we have is the noisy label produced by distant supervision. Therefore, the prediction $\mathbf{p}$ does not guarantee to match the true relation distribution. To handle this situation, instead of modeling the noise at the very beginning of the training, we use curriculum learning to guide the model to gradually learn to model the noise, and use trace regularization to control the behavior of transition matrix. Furthermore, if we have prior knowledge to roughly separate the data into reliable and unreliable parts, our training procedure can use the split as indirect supervision over the noise pattern to help the transition matrix to better model the nosie.
